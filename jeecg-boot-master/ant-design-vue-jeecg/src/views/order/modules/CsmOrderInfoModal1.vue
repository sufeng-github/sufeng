    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="病床号" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <j-dict-select-tag type="list" v-decorator="['bednum']" :trigger-change="true" dictCode="" placeholder="请选择病床号"/>
        </a-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'opeId','date','toolId','scrId','hospitalId','patient','code','bednum','companyId','customerId','status'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'opeId','date','toolId','scrId','hospitalId','patient','code','bednum','companyId','customerId','status'))
      }
