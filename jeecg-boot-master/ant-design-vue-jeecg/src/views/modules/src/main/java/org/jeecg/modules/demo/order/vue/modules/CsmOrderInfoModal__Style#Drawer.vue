    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="工具箱" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <j-dict-select-tag type="list" v-decorator="['tooId']" :trigger-change="true" dictCode="" placeholder="请选择工具箱"/>
        </a-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'date','name','patient','code','bednum','status','hospitalId','opeId','tooId','scrId','companyId','deliverymanId','followerId','salerId','fetcherId','customerId'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'date','name','patient','code','bednum','status','hospitalId','opeId','tooId','scrId','companyId','deliverymanId','followerId','salerId','fetcherId','customerId'))
      }
