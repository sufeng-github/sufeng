...
/*			function fmaterOperate(value,row,index){
				...
				function fmaterColors(value,row,index){ 	
	    	 		if(value == '100%'){
						return  '<span style="color:green">'+value+'</span>';
					}else {
						return  '<span style="color:red">'+value+'</span>';								
				...
				function fmaterColor(value,row,index){ 
					if(value == undefined){
						return '';
					}else{	
	    	 			return  '<span style="color:red">'+value+'</span>';
				...
				function indexSave(rowIndex,entityUrl,saveUrl,msg) {
					$.ajax({
						type : "post",
						url : entityUrl,
						traditional : true,
	...
	function saveOrDelete(target,entityUrl,saveUrl,msg){
							...
							$.ajax({  
								...
								success: function(result) {
							     ...
							     // alert(JSON.stringify(row));
							       	if((row.idc==null || row.idc==0) && msg.indexOf('删除')>-1){ 
				...
				function ajaxObj(url,json,msg,rowIndex){
					$.ajax({
						type : "post",
						url : url,
						data : json,
