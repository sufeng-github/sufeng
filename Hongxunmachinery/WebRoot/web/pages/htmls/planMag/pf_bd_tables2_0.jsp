 		...
 		<div id="tb" style="padding:0 10px;height:auto">	
			<div id="ttdiv" style="width:100%; height:50px;">	
				<div style="margin-top:10px; margin-left:10px; float:left">	
					<a href="javascript:;" class="easyui-linkbutton" style="height:30px;" onclick="$('#importExcelDlg').dialog('open').dialog('setTitle','导入表格')">导入</a> 
				</div>
				<div style="margin-top:10px; margin-left:10px; float:left">			 	  
		...
		$(function(){
			$('#tt').datagrid({
				...
				columns:[[
					...
					//{field:'poCreateDate',title:'定单创建日期',width:100,align:'center'},
		...
		function deleterow(target){
			$.messager.confirm('Confirm','是否删除?',function(r){
				if (r){	
					...
					if(row.status.indexOf('库')>-1){
						alert('定单存在出入库信息，禁止删除。');
						return;
					}			
					$.ajax({  
						type: "post",  
