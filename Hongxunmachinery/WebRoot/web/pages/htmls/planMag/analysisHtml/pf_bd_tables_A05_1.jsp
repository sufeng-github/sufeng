	...
	<script>																	
		$(function(){
			$('#tt').datagrid({
				...
				height:'100%',
				toolbar:'#tb',
				url:'${pageContext.request.contextPath}/bomTree.htm/analysisItem ?name=list5_1',
				singleSelect:false,
				fitColumns:true,
				autoRowHeight:false,
				striped:true,
...
/*  		$(function(){				
		...
		function newWorkNum(){
			...
			userList.push({remark:'732下料',status:window.parent.window.getTimeStamp()});
		    for(var i=0; i<rows.length; i++){  
	...
	function newWeiwaiNum(){
		var rows = $('#tt').datagrid('getSelections');  	
			...
			userList.push({remark:'732下料委外',status:window.parent.window.getTimeStamp()});
			for(var i=0; i<rows.length; i++){
				userList.push(rows[i]);	
			}
				//alert(JSON.stringify(userList));
