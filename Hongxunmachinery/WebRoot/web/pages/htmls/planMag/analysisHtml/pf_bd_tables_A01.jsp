...
<!-- 	    <div data-options="iconCls:'icon-save'" onclick="newPurchaseNum()">生成外购单</div>
	...
	<script>																
		$(function(){
			$('#tt').datagrid({
				...
				height:'100%',
				toolbar:'#tb',
				//url:'${pageContext.request.contextPath}/bomTree.htm/analysisItem ?name=list1',
				singleSelect:false,
				fitColumns:true,
				autoRowHeight:false,
				striped:true,
	...
	<script>
        ...
        */
		...
		function newWorkNum(){
			...
			userList.push({remark:'A01焊接下料',status:window.parent.window.getTimeStamp()});
		    for(var i=0; i<rows.length; i++){  
	...
	function newWeiwaiNum(){
		var rows = $('#tt').datagrid('getSelections');  	
			...
			userList.push({remark:'A01焊接下料委外',status:window.parent.window.getTimeStamp()});
			for(var i=0; i<rows.length; i++){
				userList.push(rows[i]);	
			}
				//alert(JSON.stringify(userList));
