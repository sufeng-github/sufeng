		...
		<div id="importExcelDlg" class="easyui-dialog" style="width:280px;height:150px;padding:10px 0px" data-options="top:10,closed:true">
			<form action='' enctype="multipart/form-data" method="post" name="fileForm"> 
		        <label >
 				...
 				<%--   	<input type="file" class="file" name="filename" id="fileId" onchange="upload('${pageContext.request.contextPath}/materialCompilation.htm/exlImport')" accept=".xls"/>
				 --%>
				</label> 			
			</form> 	
		</div> 
	...
	<script>
        $(function(){				
           ...
           // $('#tt').datagrid({data:getData('#tt','${pageContext.request.contextPath}/materialStock.htm/loadData')}).datagrid('clientPaging');                 
           	$.ajax({ 
				type: "post",  				
				url:'${pageContext.request.contextPath}/materialCompilation.htm/loadData',
				traditional: true,
				dataType:"json",  
				beforeSend: function () {
					load();
			...
			onSelect : function(record) {
				...
				$.messager.confirm('Confirm', '是否盘点?', function(r) {
					if (r) {
		            	...
		            	$.ajax({  
							type: "post",  
							url: '${pageContext.request.contextPath}/materialCompilation.htm/scanMaterialCompilation',			
							data:record, 
							traditional: true,
							dataType:"json", 
							beforeSend: function () {
				...
				reader.onload = function(e) {
					...
					if(result[0].indexOf('物料编码')>0 && result[0].indexOf('盘点量')>0){
						...
						if((result.length-1) != data.length){
							...
							$.ajax({
								type : "post",
								url : '${pageContext.request.contextPath}/materialCompilation.htm/xlf?date=' + date,
