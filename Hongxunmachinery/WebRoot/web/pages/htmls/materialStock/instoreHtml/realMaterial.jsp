...
<head>
    ...
    <script type="text/javascript" src="../../../js/myjs/materialStock.js"></script>  
...
<body>
	<div class="easyui-layout" data-options="fit:true" style="padding:5px;background:#eee;">	
 		<div id="tb" style="padding:0 0px;height:40px;">	
			...
			<div style="margin-top:5px; margin-left:10px; float:left">	
						<table>
							<tr>
								...
								<td><a href="javascript:;" class="easyui-linkbutton"  onclick="insert()" >新增</a></td>
							</tr>
						</table>						
			 </div> 	   
		</div> 
		<table id="tt" class="easyui-datagrid" style="width: 100%; height: 100%" 
			...
			<thead>
				<tr>
					...
					<th data-options="field:'materialNum',align:'center',sortable:true,width:20,editor:'text'">物料编码</th>								
					<th data-options="field:'specification',align:'center',sortable: true,width:80,editor:'text'">型号/规格</th>		
					...
					<th data-options="field:'alarm',align:'center',sortable:true,width:20,formatter:fmaterColor">报警</th>
					<th data-options="field:'status',align:'center',width:30,formatter:fmaterColor">状态</th>
	...
	<script>
		...
		$('#tt').datagrid({
			loadFilter: function(data){										
 				for(var i=0; i<data.rows.length;i++){
					var safeQuantity = 0;
					var itemQuantity = 0;
					var inRoadQuantity = 0;
					var quantity = 0;
					if(data.rows[i].safeQuantity !=null){
						safeQuantity =data.rows[i].safeQuantity
					}
					if(data.rows[i].itemQuantity !=null){
						itemQuantity =data.rows[i].itemQuantity
					}
					if(data.rows[i].inRoadQuantity !=null){
						inRoadQuantity =data.rows[i].inRoadQuantity
					}
					if(data.rows[i].quantity !=null){
						quantity =data.rows[i].quantity
					}
					if(quantity + inRoadQuantity - itemQuantity < safeQuantity){
						data.rows[i].alarm='报警';
					}else{
						data.rows[i].alarm='';
					}
		...
		function submitPurchaseNum(){
			action('${pageContext.request.contextPath}/materialStock.htm/getEntity', '${pageContext.request.contextPath}/purchaseDeNum.htm/newPurchaseDeNum','是否提单？');
		}
