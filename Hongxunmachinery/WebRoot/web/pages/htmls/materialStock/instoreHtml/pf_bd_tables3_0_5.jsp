		...
		$(function(){
			$('#tt').datagrid({
				...
				columns:[[
					...
					{field:'stockQuantity',title:'库存数量',align:'center',width:40},		 
	...
	<script>
        $(function(){				
          	//$('#tt').datagrid({data:getData('#tt','${pageContext.request.contextPath}/purchaseItemInStock.htm/loadData')}).datagrid('clientPaging');                 
       		...
       		$.ajax({  
				type: "post",  		
				url: '${pageContext.request.contextPath}/materialNoLimitInStock.htm/loadData',
...
/* 		function isOnlyRead(){
		...
		function inputEnter() {
			...
			$.ajax({
				type : "post",
				url : '${pageContext.request.contextPath}/materialNoLimitInStock.htm/scanInMaterialNum',
				data : data,
				traditional : true,
				dataType : "json",
				beforeSend : function() {
		...
		$('#userId').combobox({
			...
			url : '${pageContext.request.contextPath}/materialNoLimitInStock.htm/autotimp', //请求数据路径				
			editable : true, //可编辑
			hasDownArrow : false, //下拉面板不关闭
			valueField : "id", //数组的键索引
			textField : "name", //数组的值索引
			//queryParams: {outStoreNumID: window.parent.window.getOutStoreNumID()},
			icons : [ {
				iconCls : 'icon-search'
			} ],
			...
			onSelect : function(record) {
				...
				$.messager.confirm('Confirm', '是否入库?', function(r) {
					if (r) {
						//record["outStoreNumID"] = window.parent.window.getOutStoreNumID();
						record["quantity"] = $("#quantityId").numberbox("getValue");
						//alert(JSON.stringify(record));
						$.ajax({
							type : "post",
							url : '${pageContext.request.contextPath}/materialNoLimitInStock.htm/searchMaterialNum',
							data : record,
							traditional : true,
							dataType : "json",
							beforeSend : function() {
							...
							success : function(result) {
								if (result[0].hasOwnProperty('error')) {
									...
									$("#userId").textbox("setValue", "");	
									$("#quantityId").numberbox("setValue", "");
									//$("#quantityId").numberbox('textbox').focus(); 
	...
	<script>
		...
		function saveRow(target){
			$.ajax({  
				type: "post",  
				url: '${pageContext.request.contextPath}/materialNoLimitInStock.htm/getEntity',
				traditional: true,
				dataType:"json", 
				...
				success: function(result) {		
					...
					$.ajax({  
						type: "post",  
						url: '${pageContext.request.contextPath}/materialNoLimitInStock.htm/updateRow',
