...
/* 							if (row.editing){							
							...
							var c = '<a href="#" onclick="instock(this)">入库</a>';
								return c;
						}
					},
					...
					{field:'action',title:'操作',width:80,align:'center',
						formatter:function(value,row,index){
							if (row.editing){							
								...
								var e = '<a href="#" onclick="editrow1(this)">编辑</a> ';
								var d = '<a href="#" onclick="deleterow1(this)">返回</a>';
								return e+d;
							}
						}
					},
	...
	<script>
		...
		function deleterow1(target){
			$.messager.confirm('Confirm','Are you sure?',function(r){
				if (r){
					...
					if(row.ID!=0){
						funOne('#tt1','${pageContext.request.contextPath}/numSevenBranchOne.htm/deleteRow',rowIndex,para);				
					}
					
					$('#tt1').datagrid('deleteRow', rowIndex);
					  
				}else{
					alert("å é¤å¤±è´¥ï¼è¯·éæ°æä½ã")
				}
			});
		}
		function saverow1(target){
			...
			para = para.split(':')[1];
			//alert(para)  		       	
	       	funOne('#tt1', '${pageContext.request.contextPath}/numSevenBranchOne.htm/saveRow', rowIndex, para);
