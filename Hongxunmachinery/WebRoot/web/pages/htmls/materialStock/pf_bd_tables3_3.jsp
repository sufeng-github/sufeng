...
<!-- 			<div data-options="region:'south',split:true" style="height:100px;padding:5px;background:#eee;">
		...
		$(function(){
			$('#tt').datagrid({
				...
				columns:[[
					...
					{field:'workNum',title:'工单编号',width:100,align:'center'},	
			...
			$('#tt2').datagrid({
				...
				columns:[[	
					{field:'Idc',title:'ID',width:20,hidden:'true'},		
					{field:'materialNum',title:'部件号',align:'center',width:60},
					{field:'spacification',title:'型号/规格',align:'center',width:120},	
					{field:'drawNum',title:'图号',align:'center',width:60},												
					{field:'quantity',title:'领料数量',align:'center',width:60},	
					{field:'realquantity',title:'实际数量',align:'center',width:60},
					{field:'unit',title:'单位',align:'center',width:60},  
					{field:'uncleID',title:'uncleID',align:'center',width:60},			
					{field:'date',title:'领料日期',align:'center',width:100},
					{field:'action',title:'操作',width:80,align:'center',
						formatter:function(value,row,index){
							if (row.editing){							
								var s = '<a href="#" onclick="saverow2(this)">领料</a> ';
	...
	<script>	
            ...
            $('#tt1').datagrid('clientPaging');  
           	$('#ss').searchbox({
			    ...
			    searcher:function(value,name){
			    	getData('#tt','${pageContext.request.contextPath}/workNum.htm/searchNum' + '?num=' + value);
			    },
			    prompt:'出库单号'
			});      
        });  
        ...
        function spreadrow(target){
			...
			userList.push({purchaseNum:row.workNum, purchaseProcedure:row.workProcedure});
			$.ajax({  
				type: "post",  
				...
				success: function(result) {	
					//alert(JSON.stringify(result));	
					if(result.length>0){	
						$('#dlg1').dialog({title: '工单号 :' + row.workNum + '领料', queryParams: {dlgPara: row.idc} }); 
						$("#dlg1").dialog("open");					
