...
<body>
 		...
 		<div id="tb" style="padding:0 10px;height:auto">	
			<div id="ttdiv" style="width:100%;  height:50px;">
				<div style="margin-top:5px; margin-left:10px; float:left">	
					...
					<a href="javascript:;"  class="easyui-linkbutton"   onclick="$('#tt').datagrid('toExcel','装配送货单.xls')">导出</a>
	...
	<script>
        $(function(){				    
       		$.ajax({  
				type: "post",  
				url: '${pageContext.request.contextPath}/deliveryItem.htm/loadData',
		...
		/* function inputEnter() {
			...
			$.ajax({
				type : "post",
				url : '${pageContext.request.contextPath}/deliveryItem.htm/searchMaterialNum',
				data : data,
				traditional : true,
				dataType : "json",
				beforeSend : function() {
			...
			onSelect : function(record) {
				...
				$.messager.confirm('Confirm', '是否出库?', function(r) {
					if (r) {
						...
						$.ajax({
							type : "post",
							url : '${pageContext.request.contextPath}/deliveryItem.htm/outstock',
							data : record,
							traditional : true,
							dataType : "json",
							beforeSend : function() {
