...
<body>
	...
	<script>
        $(function(){				    
       		$.ajax({  
				type: "post",  
				url: '${pageContext.request.contextPath}/deliveryNum.htm/loadData',
		...
		function inputEnter() {
			...
			$.ajax({
				type : "post",
				url : '${pageContext.request.contextPath}/deliveryNum.htm/searchDeliveryNum',
				data : data,
				traditional : true,
				dataType : "json",
				beforeSend : function() {
	...
	<script>
				...
				//alert(JSON.stringify($('#tt').datagrid('getData')));
					...
					$('#tt').datagrid('selectRow',rowIndex);
	       			...
	       			if(row.idc!=0){
	       				...
	       				$.ajax({  
							type: "post",  
							url: '${pageContext.request.contextPath}/deliveryNum.htm/deleteRow',
							data:{idc:row.idc}, 
							traditional: true,
							dataType:"json", 
							//contentType : 'application/json;charset=utf-8', //设置请求头信息  
		...
		function saverow(target){
			...
			$.ajax({  
				type: "post",  
				url: '${pageContext.request.contextPath}/deliveryNum.htm/getEntity',
				traditional: true,
				dataType:"json", 
				//contentType : 'application/json;charset=utf-8', //设置请求头信息  
				success: function(result) {						
					...
					$.ajax({  
						type: "post",  
						url: '${pageContext.request.contextPath}/deliveryNum.htm/saveRow',
		...
		function spreadrow(target){
	        ...
	        $("#dialog").dialog({
			     ...
			     content:"<iframe scrolling='auto' frameborder='0' src='./pf_bd_tables4_2_5_0.jsp' style='width:100%; height:100%; display:block;'></iframe>"
			});	
		...
		$("#dialog").dialog({
			onClose : function() {
				...
				$.ajax({
					type : "post",
					url : '${pageContext.request.contextPath}/deliveryNum.htm/changeStatus',
					data : {
		...
		function search(){
	        ...
	        $("#dialog").dialog({
			     ...
			     content:"<iframe scrolling='auto' frameborder='0' src='./pf_bd_tables4_2_5_1.jsp' style='width:100%; height:100%; display:block;'></iframe>"
			});	
			$("#dialog").dialog("open"); // 打开dialog     	
        }
