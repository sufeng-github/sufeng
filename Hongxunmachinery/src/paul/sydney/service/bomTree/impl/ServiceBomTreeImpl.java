...
public class ServiceBomTreeImpl implements ServiceBomTreeInf {
	...
	@Override
	public List<Map<String, Object>> analysis(List<HongXunPoN> hongXunPoNList) {
		// TODO Auto-generated method stub
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
	...
	public boolean getTablesStatus(String str) {
		...
		HongXunTablesStatus hongXunTablesStatus = stockDao.tablesStatusFindById(1);
		if (hongXunTablesStatus == null) {
			...
			stockDao.save(hongXunTablesStatus);
		}
	...
	public void setTablesStatus(String str, boolean flag) {
		...
		stockDao.update(hongXunTablesStatus);
	}
	...
	private List<HongXunPoN> analysisOne(List<HongXunPoN> hongXunPoNList, List<HongXunPonNum> hongXunPonNumList) {
		...
		List<HongXunSpecialMaterialNum> set = stockDao.quary(hongXunSpecialMaterialNum);
		...
		for (HongXunPoN item : hongXunNewPoNList) {
			...
			if (hongXunBomTrees.size() == 0) {
				...
				if (hongXunBomTrees.get(0).getSn() == 1) {
					...
					// System.out.println(hongXunBomTree.getBomMaterialNum() + "
					// : " + hongXunBomTree.getBomGroup());
					if (item.getWorkQuantity() != 0) {
						if (hongXunBomTrees.get(0).getBomGroup().equals("A01焊接下料")) {
							...
							/*
							 * if(listAssemble==null){ listAssemble = new
							 * ArrayList<HongXunBomTree>(); }
							 */
							listAssemble.add(hongXunBomTrees.get(0));
						} else if (hongXunBomTrees.get(0).getBomGroup().equals("732")) {
							/*
							 * if(list732==null){ list732 = new
							 * ArrayList<HongXunBomTree>(); }
							 */
							// hongXunBomTree.setBomItemQuantity(item.getQuantity());
							// hongXunBomTree.setBomDeliveryDate(new
							// SimpleDateFormat("yyyy-MM-dd").format(item.getPoDeliveryDate()));
